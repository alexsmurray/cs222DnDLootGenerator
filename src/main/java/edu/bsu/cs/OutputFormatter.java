...
public class OutputFormatter {
    ...
    protected static String formatItemForList(Item item) {
        ...
        formattedLine += String.format("%-10s",capitalize(item, "Name"));
        formattedLine += String.format("%10s",capitalize(item, "Rarity"));
        formattedLine += String.format("%10s",capitalize(item, "Type"));
        if (item.getAttunement().equals("True")) {
            formattedLine += String.format("%10s","*");
        }
